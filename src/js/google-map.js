function initMap() {	   
	...
	var _coord_str = $('body').eq(0).attr('data-azbn7__mdl__map__coord') || '';
	
	if(_coord_str != '') {
		
		var _coord = JSON.parse(_coord_str);
		
		var coordOffice = _coord.center;//{lat: 52.9710326, lng: 36.0639794},
			zoom_map_office = 17,
			mapOptionsOffice = {
		...
		var	orgOfficeCoord = _coord.point;//{lat: 52.971136, lng: 36.066790};

		var 
			orgOffice = new google.maps.Marker({
				position: orgOfficeCoord,
				map: mapOffice,
				icon: iconOffice
			});
